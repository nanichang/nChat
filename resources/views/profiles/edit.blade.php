	...
	<div class="col-sm-9 col-lg-10 light bg">
			<div class="tab-content pos-rlt">
				<div class="tab-pane active" id="tab-1">
					...
					<form action="{{ route('profile.update') }}" method="post" role="form" enctype="multipart/form-data" class="p-a-md col-md-6">
						{{ csrf_field() }}
						...
						<div class="form-group">
							...
							<input type="text" name="facebook_url" value="{{ $user->facebook_url }}" class="form-control" required>
						</div>
						<div class="form-group">
							<label>Twitter ID</label>
							<input type="text" name="twitter_url" value="{{ $user->twitter_url }}" class="form-control" required>
						</div>
						<div class="form-group">
							<label>Google+</label>
							<input type="text" name="google_plus" value="{{ $user->google_plus }}" class="form-control" required>
						</div>
						<div class="form-group">
							<label>LinkdIn Address</label>
							<input type="text" name="linkdin_url" value="{{ $user->linkdin_url }}" class="form-control" required>
						</div>
						<div class="form-group">
							<label>About Me</label>
							<textarea name="about" class="form-control" rows="3" required> {{ $user->about }}</textarea>
						</div>
						<div class="form-group">
							<label for="address">Address</label>
							<textarea name="address" class="form-control" rows="3" required>{{ $user->address }}</textarea>
						</div>
						<button type="submit" class="btn btn-lg btn-info m-t">Update My Profile</button>
					</form>
				</div>
